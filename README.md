...
> 状态：可行性评估 + 设计方案（待你确认后再落地部署）
> 目标：全自动 —— 北京时间每天 18:00 采集 → AI 生成日报 → 直接发布到 WordPress「日报」类目
...
│    └─ 调 ai.jinbufenzi.com/v1（OpenAI 兼容，model=auto）       │
...
- 需要在仓库 `Settings → Secrets and variables → Actions` 里配置：
  - `AI_SIDEHUSTLE_API_KEY` = `sk-4c7ab...`（你的 AI key）
  - `WP_APP_PASSWORD` = `zila46...`（WordPress 应用密码）
